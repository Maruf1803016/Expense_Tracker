...
|---|---|---|---|
| 1 | Recurring income and bill schedules | The web prototype does not yet automate regular paydays/bills or show upcoming reminders | Create/edit/disable schedules; calculate next due dates correctly; surface upcoming items; record paid/received outcome |
| 2 | Advanced reports and real exports | Current export feedback is a local placeholder, not a user-downloadable reporting workflow | Date-range/account/category filters; CSV and PDF file generation; accurate totals matching the filtered ledger |
| 3 | Firebase authentication and Firestore persistence | The current implementation is intentionally local-state only, so data is not durable or synchronized | Email/password authentication; protected user scope; Firestore CRUD; loading/error/retry states; existing Flutter data-model alignment |
| 4 | Import and robust account management | The app needs an onboarding/import path for real accounts, balances, and historical transactions | CSV import preview and validation; duplicate handling; account number masking; safe balance reconciliation |
| 5 | Final release audit | Visual and functional quality must be tested as a complete product rather than as isolated feature slices | Complete every row in this document on mobile and desktop; log defects; re-test fixes; only then mark release-ready |
