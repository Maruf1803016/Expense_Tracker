/**
 * Ink & Ledger page: a warm-paper financial fieldbook where every number is derived
 * from the local transaction ledger. Keep dense surfaces crisp and evidence-led.
 */
...
type Account = { id: string; name: string; kind: AccountKind; startingBalance: number; color: string };
type Category = { id: string; name: string; monthlyBudget: number; color: string; icon: IconKey };
...
const seedCategories: Category[] = [
  { id: "food", name: "Food & Dining", monthlyBudget: 800, color: "#b78a3d", icon: "food" },
  { id: "home", name: "Home & Utilities", monthlyBudget: 1400, color: "#607b69", icon: "home" },
  { id: "travel", name: "Travel", monthlyBudget: 1000, color: "#c77c5f", icon: "travel" },
  { id: "personal", name: "Personal", monthlyBudget: 400, color: "#92769b", icon: "shopping" },
  { id: "income", name: "Freelance income", monthlyBudget: 0, color: "#496d56", icon: "income" },
];
...
// Core source of truth: every dashboard, account, budget, goal and trip amount is calculated from these records.
const seedTransactions: Transaction[] = [
  { id: "t01", type: "income", amount: 2800, categoryId: "income", accountId: "daily", date: "2026-03-04", merchantNote: "Consulting project", icon: "income" },
  { id: "t02", type: "expense", amount: 145, categoryId: "personal", accountId: "daily", date: "2026-03-08", merchantNote: "Studio supplies", icon: "shopping" },
  { id: "t03", type: "income", amount: 2200, categoryId: "income", accountId: "daily", date: "2026-04-02", merchantNote: "Northline Studio", icon: "income" },
  { id: "t04", type: "expense", amount: 96, categoryId: "travel", accountId: "credit", date: "2026-04-17", merchantNote: "Rail pass", icon: "travel", tag: { tripId: "spring" } },
  { id: "t05", type: "income", amount: 2600, categoryId: "income", accountId: "daily", date: "2026-05-03", merchantNote: "Client retainer", icon: "income" },
  { id: "t06", type: "expense", amount: 320, categoryId: "food", accountId: "daily", date: "2026-05-18", merchantNote: "Groceries", icon: "food" },
  { id: "t07", type: "income", amount: 1700, categoryId: "income", accountId: "daily", date: "2026-06-08", merchantNote: "Workshop honorarium", icon: "income" },
  { id: "t08", type: "expense", amount: 570, categoryId: "home", accountId: "daily", date: "2026-06-13", merchantNote: "Home insurance", icon: "home" },
  { id: "t09", type: "transfer", amount: 2200, accountId: "daily", destinationAccountId: "reserve", date: "2026-06-23", merchantNote: "Contribution to Quiet reserve", icon: "transfer", tag: { goalId: "reserve-goal" } },
  { id: "t10", type: "income", amount: 2500, categoryId: "income", accountId: "daily", date: "2026-07-03", merchantNote: "Northline Studio", icon: "income" },
  { id: "t11", type: "expense", amount: 210, categoryId: "personal", accountId: "daily", date: "2026-07-12", merchantNote: "Field books", icon: "shopping" },
  { id: "t12", type: "transfer", amount: 2680, accountId: "daily", destinationAccountId: "reserve", date: "2026-07-21", merchantNote: "Contribution to Quiet reserve", icon: "transfer", tag: { goalId: "reserve-goal" } },
  { id: "t13", type: "transfer", amount: 800, accountId: "daily", destinationAccountId: "reserve", date: "2026-07-27", merchantNote: "Contribution to Quiet reserve", icon: "transfer", tag: { goalId: "reserve-goal" } },
  { id: "t14", type: "transfer", amount: 1600, accountId: "daily", destinationAccountId: "reserve", date: "2026-07-30", merchantNote: "Contribution to A place of our own", icon: "transfer", tag: { goalId: "home-goal" } },
  { id: "t15", type: "expense", amount: 18.5, categoryId: "personal", accountId: "daily", date: "2026-08-13", merchantNote: "Field Notes", icon: "shopping" },
  { id: "t16", type: "expense", amount: 980, categoryId: "home", accountId: "daily", date: "2026-08-14", merchantNote: "Studio Rent", icon: "home" },
  { id: "t17", type: "expense", amount: 214, categoryId: "travel", accountId: "daily", date: "2026-08-15", merchantNote: "Family weekend lodging", icon: "travel", tag: { tripId: "family" } },
  { id: "t18", type: "expense", amount: 126, categoryId: "travel", accountId: "credit", date: "2026-08-15", merchantNote: "Rail tickets — Kyoto", icon: "travel", tag: { tripId: "spring" } },
  { id: "t19", type: "expense", amount: 46.8, categoryId: "food", accountId: "daily", date: "2026-08-16", merchantNote: "Market & Morning", icon: "food" },
  { id: "t20", type: "income", amount: 1840, categoryId: "income", accountId: "daily", date: "2026-08-16", merchantNote: "Northline Studio", icon: "income" },
  { id: "t21", type: "transfer", amount: 500, accountId: "daily", destinationAccountId: "credit", date: "2026-08-12", merchantNote: "Credit card payment", icon: "transfer" },
];
...
export default function Home() {
  ...
  const categorySpent = useMemo(() => Object.fromEntries(seedCategories.map((category) => [category.id, periodTransactions.filter((transaction) => transaction.type === "expense" && transaction.categoryId === category.id).reduce((sum, transaction) => sum + transaction.amount, 0)])), [periodTransactions]);
  ...
  const filteredTransactions = useMemo(() => transactions.filter((transaction) => {
    ...
    const matchesCategory = !categoryFilterId || transaction.categoryId === categoryFilterId;
    const term = query.toLowerCase();
    const category = seedCategories.find((item) => item.id === transaction.categoryId)?.name.toLowerCase() ?? "";
    return matchesType && matchesCategory && (!term || transaction.merchantNote.toLowerCase().includes(term) || category.includes(term));
  }).sort((a, b) => b.date.localeCompare(a.date)), [transactions, filter, categoryFilterId, query]);
  ...
  function openTransactionEditor(transaction: Transaction) {
    openTransactionDraft({
      ...
      categoryId: transaction.categoryId ?? "personal",
  ...
  function saveDraft() {
    if (draft === "transaction") {
      ...
      const category = seedCategories.find((item) => item.id === transactionDraft.categoryId);
    ...
    if (draft === "goal") {
      ...
      setGoals((current) => [...current, { id: `goal-${Date.now()}`, name: draftTitle.trim() || "New savings goal", targetAmount: amount, targetDate: "No target date", icon: "goal" }]);
    ...
    <div className="app-shell">
      ...
      <main className="content">
        ...
        <section className="page-enter" key={`${active}-${detail?.id ?? "root"}`}>
          {active === "Overview" && <OverviewView balance={availableBalance} netWorth={netWorth} accounts={accountBalances} totals={totals} categories={seedCategories} categorySpent={categorySpent} transactions={filteredTransactions} filter={filter} categoryFilterId={categoryFilterId} query={query} onFilter={setFilter} onQuery={setQuery} onClearCategory={() => setCategoryFilterId(null)} onOpenCategory={openCategory} onOpenDraft={() => openTransactionDraft()} onEdit={openTransactionEditor} />}
          {active === "Insights" && <InsightsView transactions={transactions} categories={seedCategories} categorySpent={categorySpent} onOpenCategory={openCategory} />}
          {active === "Horizon" && (detail ? <PlanDetailView detail={detail} goals={goals} trips={trips} goalProgress={goalProgress} tripSpend={tripSpend} transactions={transactions} categories={seedCategories} accounts={seedAccounts} onBack={() => setDetail(null)} onEdit={openTransactionEditor} onAddGoalFunds={(goal) => openTransactionDraft({ type: "transfer", accountId: "daily", destinationAccountId: "reserve", goalId: goal.id, merchantNote: `Contribution to ${goal.name}` })} onLogTripExpense={(trip) => openTransactionDraft({ type: "expense", accountId: "daily", categoryId: "travel", tripId: trip.id, merchantNote: `${trip.name} expense` })} /> : <HorizonView tab={horizon} onTab={setHorizon} goals={goals} trips={trips} goalProgress={goalProgress} tripSpend={tripSpend} onCreateGoal={() => setDraft("goal")} onCreateTrip={() => setDraft("trip")} onOpenGoal={(goal) => setDetail({ kind: "goal", id: goal.id })} onOpenTrip={(trip) => setDetail({ kind: "trip", id: trip.id })} />)}
          {active === "Settings" && <SettingsView accounts={accountBalances} />}
        </section>
      </main>
      ...
      {draft && <DraftPanel kind={draft} title={draftTitle} amount={draftAmount} transaction={transactionDraft} accounts={seedAccounts} categories={seedCategories} goals={goals} trips={trips} onTitle={setDraftTitle} onAmount={setDraftAmount} onTransaction={setTransactionDraft} onClose={resetDraft} onSave={saveDraft} onDelete={deleteDraftTransaction} />}
    </div>
  );
}

function OverviewView({ balance, netWorth, accounts, totals, categories, categorySpent, transactions, filter, categoryFilterId, query, onFilter, onQuery, onClearCategory, onOpenCategory, onOpenDraft, onEdit }: {
  ...
  onFilter: (value: "all" | TransactionType) => void; onQuery: (value: string) => void; onClearCategory: () => void; onOpenCategory: (id: string) => void; onOpenDraft: () => void; onEdit: (transaction: Transaction) => void;
}) {
  ...
  const plannedBudget = categories.reduce((sum, category) => sum + category.monthlyBudget, 0);
  return <>
    <header className="page-header"><div><div className="page-kicker">Personal finance, considered</div><h1>Keep the whole picture<br />in view.</h1><p className="page-subtitle">A clear fieldbook for the money that moves your days.</p></div><button className="add-button" onClick={onOpenDraft}><Plus size={17} /><span>Add transaction</span></button></header>
...
function InsightsView({ transactions, categories, categorySpent, onOpenCategory }: { transactions: Transaction[]; categories: Category[]; categorySpent: Record<string, number>; onOpenCategory: (id: string) => void }) {
  ...
  const mix = categories.filter((category) => category.monthlyBudget > 0 && (categorySpent[category.id] ?? 0) > 0).sort((a, b) => (categorySpent[b.id] ?? 0) - (categorySpent[a.id] ?? 0));
...
function HorizonView({ tab, onTab, goals, trips, goalProgress, tripSpend, onCreateGoal, onCreateTrip, onOpenGoal, onOpenTrip }: { tab: "Goals" | "Plans"; onTab: (value: "Goals" | "Plans") => void; goals: Goal[]; trips: Trip[]; goalProgress: Record<string, number>; tripSpend: Record<string, number>; onCreateGoal: () => void; onCreateTrip: () => void; onOpenGoal: (goal: Goal) => void; onOpenTrip: (trip: Trip) => void }) {
  return (
    <>
      ...
      {tab === "Goals" ? <><div className="page-header compact-header"><div><div className="page-kicker">Set aside with intention</div><h1>Your quiet reserves.</h1></div><button className="add-button" onClick={onCreateGoal}><Plus size={16} /><span>New goal</span></button></div><div className="horizon-grid">{goals.map((goal) => <GoalCard goal={goal} saved={goalProgress[goal.id] ?? 0} key={goal.id} onOpen={() => onOpenGoal(goal)} />)}</div></> : <><div className="page-header compact-header"><div><div className="page-kicker">Budget the memory, not the aftermath</div><h1>Plans with room to enjoy.</h1></div><button className="add-button" onClick={onCreateTrip}><Plus size={16} /><span>New plan</span></button></div><div className="horizon-grid">{trips.map((trip) => <TripCard trip={trip} spent={tripSpend[trip.id] ?? 0} key={trip.id} onOpen={() => onOpenTrip(trip)} />)}</div></>}
    </>
  );
}
...
function PlanDetailView({ detail, goals, trips, goalProgress, tripSpend, transactions, categories, accounts, onBack, onEdit, onAddGoalFunds, onLogTripExpense }: { detail: Detail; goals: Goal[]; trips: Trip[]; goalProgress: Record<string, number>; tripSpend: Record<string, number>; transactions: Transaction[]; categories: Category[]; accounts: Account[]; onBack: () => void; onEdit: (transaction: Transaction) => void; onAddGoalFunds: (goal: Goal) => void; onLogTripExpense: (trip: Trip) => void }) {
  ...
  return <><button className="detail-back" onClick={onBack}><ArrowLeft size={16} /> Back to Horizon</button><header className="page-header detail-header"><div><div className="page-kicker">{isGoal ? "Savings goal" : "Trip & event plan"}</div><h1>{title}</h1><p className="page-subtitle">{isGoal ? (subject as Goal).targetDate : (subject as Trip).dateRange}</p></div><button className="add-button" onClick={() => isGoal ? onAddGoalFunds(subject as Goal) : onLogTripExpense(subject as Trip)}><Plus size={16} /><span>{isGoal ? "Add funds" : "Log expense"}</span></button></header><section className="paper-card detail-summary"><div><span className="detail-label">{isGoal ? "Held aside" : "Spent so far"}</span><strong>{fmt.format(accumulated)}</strong></div><div><span className="detail-label">{isGoal ? "Target" : "Budget"}</span><strong>{fmt.format(target)}</strong></div><div><span className="detail-label">{isGoal ? "Still to fund" : "Remaining"}</span><strong>{fmt.format(Math.max(0, target - accumulated))}</strong></div></section><section className="paper-card section-card detail-ledger"><div className="section-head"><h2>Tagged ledger</h2><span className="page-kicker" style={{ margin: 0 }}>{records.length} records</span></div><div className="transaction-list">{records.length ? records.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} categories={categories} onEdit={onEdit} />) : <p className="budget-note">No linked transactions yet. Use the add action above to pre-fill the appropriate ledger tag.</p>}</div></section><section className="paper-card detail-note"><PiggyBank size={18} /><p>{isGoal ? "Funding this goal creates a transfer tagged to the goal. It moves money between accounts without changing your net worth." : "Logging an expense assigns the trip tag directly to a normal expense transaction, so trip spend and category budgets stay in agreement."}</p><span>{accounts.length} accounts remain in the same ledger.</span></section></>;
}

function SettingsView({ accounts }: { accounts: Array<Account & { balance: number }> }) {
  return <><header className="page-header"><div><div className="page-kicker">Housekeeping</div><h1>The ledger<br />behind the ledger.</h1><p className="page-subtitle">Your accounts and categories are the vocabulary every calculation reads.</p></div></header><div className="settings-grid"><article className="paper-card settings-card"><div className="set-icon"><WalletCards size={19} /></div><h3>Accounts</h3><p>{accounts.filter((account) => account.kind === "asset").length} assets and {accounts.filter((account) => account.kind === "liability").length} liability are contributing to net worth correctly.</p><button className="secondary-button">Manage accounts</button></article><article className="paper-card settings-card"><div className="set-icon"><CreditCard size={19} /></div><h3>Categories</h3><p>Food, home, travel, and personal budgets derive their spent value from expense records.</p><button className="secondary-button">Edit categories</button></article><article className="paper-card settings-card"><div className="set-icon"><FileDown size={19} /></div><h3>Export data</h3><p>Prepare a CSV or an archival PDF that keeps transactions, goals, and trips together.</p><button className="secondary-button">Prepare export</button></article><article className="paper-card settings-card"><div className="set-icon"><Check size={19} /></div><h3>Monthly rhythm</h3><p>Your currency is USD. Budgets reset on the first day of every month.</p><button className="secondary-button">Review preferences</button></article></div></>;
}

function DraftPanel({ kind, title, amount, transaction, accounts, categories, goals, trips, onTitle, onAmount, onTransaction, onClose, onSave, onDelete }: { kind: Exclude<DraftKind, null>; title: string; amount: string; transaction: TransactionDraft; accounts: Account[]; categories: Category[]; goals: Goal[]; trips: Trip[]; onTitle: (value: string) => void; onAmount: (value: string) => void; onTransaction: (value: TransactionDraft | ((current: TransactionDraft) => TransactionDraft)) => void; onClose: () => void; onSave: () => void; onDelete: () => void }) {
  const heading = kind === "transaction" ? (transaction.id ? "Edit transaction" : "Draft a transaction") : kind === "goal" ? "Set a new goal" : "Plan the journey";
  const descriptor = kind === "transaction" ? "Record, recategorise, or correct a money movement." : kind === "goal" ? "Give future money a purpose." : "Set a ceiling before the memories start.";
  const update = (patch: Partial<TransactionDraft>) => onTransaction((current) => ({ ...current, ...patch }));
  return <div className="draft-backdrop" role="dialog" aria-modal="true" aria-label={heading} onMouseDown={onClose}><aside className="draft-panel" onMouseDown={(event) => event.stopPropagation()}><div className="draft-top"><div><div className="draft-kicker">{descriptor}</div><h2>{heading}</h2></div><button className="close-button" onClick={onClose} aria-label="Close"><X size={17} /></button></div>{kind === "transaction" && <><div className="form-field"><label>Movement</label><div className="type-options">{(["expense", "income", "transfer"] as const).map((type) => <button key={type} className={`type-option ${transaction.type === type ? "active" : ""}`} onClick={() => update({ type })}>{type[0].toUpperCase() + type.slice(1)}</button>)}</div></div><div className="form-field"><label>Merchant or note</label><input value={transaction.merchantNote} onChange={(event) => update({ merchantNote: event.target.value })} placeholder={transaction.type === "transfer" ? "e.g. Contribution to reserve" : "e.g. Sunday market"} autoFocus /></div><div className="form-field"><label>Amount</label><input value={transaction.amount} onChange={(event) => update({ amount: event.target.value.replace(/[^0-9.]/g, "") })} placeholder="0.00" inputMode="decimal" /></div><div className="form-field"><label>{transaction.type === "transfer" ? "From account" : "Account"}</label><select value={transaction.accountId} onChange={(event) => update({ accountId: event.target.value })}>{accounts.map((account) => <option value={account.id} key={account.id}>{account.name} · {account.kind}</option>)}</select></div>{transaction.type === "transfer" && <div className="form-field"><label>To account</label><select value={transaction.destinationAccountId} onChange={(event) => update({ destinationAccountId: event.target.value })}>{accounts.filter((account) => account.id !== transaction.accountId).map((account) => <option value={account.id} key={account.id}>{account.name} · {account.kind}</option>)}</select></div>}{transaction.type !== "transfer" && <div className="form-field"><label>Category</label><select value={transaction.categoryId} onChange={(event) => update({ categoryId: event.target.value })}>{categories.filter((category) => transaction.type === "income" ? category.id === "income" : category.id !== "income").map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select></div>}<div className="form-field"><label>Date</label><input type="date" value={transaction.date} onChange={(event) => update({ date: event.target.value })} /></div><div className="tag-grid"><div className="form-field"><label>Goal tag</label><select value={transaction.goalId} onChange={(event) => update({ goalId: event.target.value })}><option value="none">No goal</option>{goals.map((goal) => <option value={goal.id} key={goal.id}>{goal.name}</option>)}</select></div><div className="form-field"><label>Trip tag</label><select value={transaction.tripId} onChange={(event) => update({ tripId: event.target.value })}><option value="none">No trip</option>{trips.map((trip) => <option value={trip.id} key={trip.id}>{trip.name}</option>)}</select></div></div></>}{kind !== "transaction" && <><div className="form-field"><label>{kind === "goal" ? "Goal title" : "Plan name"}</label><input value={title} onChange={(event) => onTitle(event.target.value)} placeholder={kind === "goal" ? "e.g. Home reserve" : "e.g. Autumn in Lisbon"} autoFocus /></div><div className="form-field"><label>{kind === "goal" ? "Target amount" : "Working budget"}</label><input value={amount} onChange={(event) => onAmount(event.target.value.replace(/[^0-9.]/g, ""))} placeholder={kind === "goal" ? "5,000" : "1,200"} inputMode="decimal" /></div></>}<div className="draft-actions"><button className="primary-button draft-submit" onClick={onSave}>{kind === "transaction" ? "Save entry" : kind === "goal" ? "Create goal" : "Create plan"}</button>{kind === "transaction" && transaction.id && <button className="delete-button" onClick={onDelete}><Trash2 size={15} /> Delete entry</button>}</div></aside></div>;
}
